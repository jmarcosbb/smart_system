    ...
    public function selecionarQuantidade($horario, $tipo)
{
    ...
    public function registrarEmbarque(Request $request)
    {
        ...
        $horario = $request->horario; // Pega o horário do formulário

        $mensagem = "Embarque registrado! Horário: <strong>{$horario}</strong>, Categoria: <strong>{$tipoDisplay}</strong>, Pessoas: <strong>{$quantidade}</strong>.";

        return view('terminal.confirmacao', ['mensagem' => $mensagem]);
    }
    ...
    public function registrarSaida(Request $request)
    {
        ...
        $horario = $request->horario; // Pode ser null
